...
<div class="navTab nav nav-tabs" id="nav-tab" role="tablist">
            ...
            <div class="btnTab video"><button class=" nav-link btnTab" id="nav-contact-tab" data-bs-toggle="tab" data-bs-target="#nav-contact" type="button" role="tab" aria-controls="nav-contact" aria-selected="false">Video</button></div>
            ...
            <div class="btnTab challenge"><button class=" active nav-link btnTab" id="nav-contact-tab" data-bs-toggle="tab" data-bs-target="#nav-contact" type="button" role="tab" aria-controls="nav-contact" aria-selected="false">Challenge</button></div>
          </div>
    <div id="encription">
...
if (isset($_SESSION['id'])) {
    $rowcount = mysqli_num_rows(mysqli_query($con, 'select * from middle  where usersID='.$_SESSION['id'].''));
        ...
        $challangecount = mysqli_num_rows(mysqli_query($con,"select * from challenges where level=$level"));
        // $challangecount = getCountQuestioninLevel($level,$con);
//  echo 'answer count '.  $_SESSION['count_answer'].'';
//  echo 'answer count '.$rowcount.'';
//  $rowcount= $_SESSION['count_answer'];
        if (isset($_SESSION['isLogin'])) {
            if ($_SESSION['isLogin'] == 1) {
                ...
                //   echo 'row Count '.$rowcount.'' ;
//  echo 'challangecount '.$challangecount.'' ;
           echo "
   ...
   <a href='home.php?level=3'>         <button type='button' class='btn btn-secondary'>Advanced</button></a>
        ...
        <div class='txtHelp'>
            <input type='text' class='txtHelp1' name='text' id='txtHelp1'>
            <button type='submit ' name='btnHelp1' class='btnHelp btn btn-primary'>send</button>
        </div>
    </div>";
  ...
  $(document).ready(function(){
    ...
    $(".txtHelp").hide();
        $(".btnHelp").click(function(){
            var txtHelp=document.getElementById("txtHelp1").value;
            // alert(xx);
            $.ajax({
                type: "POST",
                url: "challenge.php",
